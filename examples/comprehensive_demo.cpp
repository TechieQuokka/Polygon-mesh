...
void demonstrate_basic_mesh_operations() {
    ...
    auto v0 = mesh.add_vertex(Vector3(0.0f, 0.0f, 0.0f));    // Base center
    auto v1 = mesh.add_vertex(Vector3(1.0f, 0.0f, 1.0f));    // Base corner 1
    auto v2 = mesh.add_vertex(Vector3(-1.0f, 0.0f, 1.0f));   // Base corner 2
    auto v3 = mesh.add_vertex(Vector3(-1.0f, 0.0f, -1.0f));  // Base corner 3
    auto v4 = mesh.add_vertex(Vector3(1.0f, 0.0f, -1.0f));   // Base corner 4
    auto v5 = mesh.add_vertex(Vector3(0.0f, 2.0f, 0.0f));    // Apex
    ...
    // Note: All faces are triangles in this example
...
void demonstrate_math_operations() {
    ...
    auto translation = Matrix4::translation(Vector3(2.0f, 3.0f, 4.0f));
    auto rotation = Matrix4::rotation_y(utils::math::degrees_to_radians(45.0f));
    auto scaling = Matrix4::scaling(Vector3(2.0f, 1.0f, 0.5f));
...
void demonstrate_file_io() {
    ...
    mesh.add_vertex(Vector3(1.0f, 0.0f, 0.0f));
    mesh.add_vertex(Vector3(0.5f, 1.0f, 0.0f));
    mesh.add_vertex(Vector3(0.5f, 0.5f, 1.0f));
    ...
    std::cout << "Created tetrahedron with " << mesh.vertex_count() 
              << " vertices and " << mesh.face_count() << " faces\n";
    
    // Save to OBJ file
    std::string filename = "tetrahedron_demo.obj";
    if (io::save_mesh(filename, mesh)) {
        std::cout << "Successfully saved mesh to " << filename << "\n";
        
        // Try to load it back
        try {
            auto loaded_mesh = io::load_mesh<float>(filename);
            std::cout << "Successfully loaded mesh: " << loaded_mesh.vertex_count() 
                      << " vertices, " << loaded_mesh.face_count() << " faces\n";
        } catch (const std::exception& e) {
            std::cout << "Failed to load mesh: " << e.what() << "\n";
        }
    } else {
        std::cout << "Failed to save mesh to " << filename << "\n";
    }
}
...
void demonstrate_profiling() {
    ...
    {
        POLYGON_MESH_PROFILE_SCOPE("mesh_creation");
    ...
    {
        POLYGON_MESH_PROFILE_SCOPE("math_operations");
        
        Vector3 result;
        for (int i = 0; i < 10000; ++i) {
            Vector3 v1(static_cast<float>(i), static_cast<float>(i + 1), static_cast<float>(i + 2));
            Vector3 v2(static_cast<float>(i * 2), static_cast<float>(i * 3), static_cast<float>(i * 4));
            result = v1.cross(v2);
        }
        
        std::cout << "Performed 10000 cross products, final result: (" 
...
int main() {
    std::cout << "=== Polygon Mesh Library Comprehensive Demo ===\n";
